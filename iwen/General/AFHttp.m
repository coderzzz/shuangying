...
//        NSDictionary *res  = [NSJSONSerialization JSONObjectWithData:responseObject options:NSJSONReadingMutableContainers error:nil];
        ...
        NSLog(@"%@,%@,%@",responseObject,operation.request.URL,params);
        if(success)
            success(responseObject);
    } failure:^(AFHTTPRequestOperation *operation, NSError *error) {
...
- (void)uploadFile:(NSString *)url withParams:(NSDictionary *)params fileURL:(NSURL *)fileUrl fileKey:(NSString *)name completionBlock:(void (^)(id obj))success failureBlock:(void (^)(NSError * error,NSString * responseString))failure
{
    [_manager POST:[[self class] urlEncode:url] parameters:params constructingBodyWithBlock:^(id<AFMultipartFormData> formData) {
        ...
        NSLog(@"%@,%@,%@",error,operation.responseString,operation.request.allHTTPHeaderFields);
        if(failure)
            failure(error,operation.responseString);
