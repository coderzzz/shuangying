...
@interface PutOnViewController ()<UITableViewDataSource,UITableViewDelegate,UIScrollViewDelegate>
...
@implementation PutOnViewController
{
...
//    self.tableview.backgroundColor = BackgroundColor;
    ...
    _segmentedControl = [[HMSegmentedControl alloc]initWithSectionTitles:@[@"红包广告",@"优惠券"]];
...
-(void)loadData
{
    ...
    __weak PutOnViewController * weakSelf = self;
...
-(void)configBlock
{
    [CircleService shareInstaced].getForumSuccess = ^(id obj){
           ...
           self.redlab.text = [NSString stringWithFormat:@"累计投放%lu个红包广告，点击量%d次",(unsigned long)tab1List.count,a];
    ...
    [CircleService shareInstaced].getCommentSuccess = ^(id obj){
        ...
        self.ticlab.text = [NSString stringWithFormat:@"累计投放%lu个优惠券，点击量%d次",(unsigned long)tab2List.count,a];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (dic) {
        ...
        cell.rigLab.text = [NSString stringWithFormat:@"%@次",dic[@"fclickConut"]];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (dic) {
        ...
        pusController *vc = [[pusController alloc]init];
        vc.token =dic[@"fid"];
        vc.title =dic[@"fname"];
